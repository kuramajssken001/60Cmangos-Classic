...
bool ChatHandler::HandleWorldChat(char* args)
{
	if (sWorld.getConfig(CONFIG_BOOL_WORLD_CHAT_ON))
	{
		...
		if (!m_session->GetPlayer()->HasItemCount(90001, 1))
		{
			PSendSysMessage(20005);
			return false;
		}
		...
		str += "|r]:";
		str += args;
		sWorld.SendWorldText(20004, GetNameLink().c_str(), args);
		player->DestroyItemCount(90001, 1, true);
		return true;
	}
	else
	{
		PSendSysMessage(20006);
	}
}
